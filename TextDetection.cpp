...
void normalizeImage (const Mat& input, Mat& output) {
	...
	float coef = (float)(254.0 / (255 * range));
    for ( int row = 0; row < input.rows; row++ ) {
...
Mat textDetection (const Mat& input, bool dark_on_light) {
	...
	Mat dstbin(input.size(), CV_8UC1);
	double otsu_thresh_val = threshold(grayImage, dstbin, 0, 255, CV_THRESH_BINARY | CV_THRESH_OTSU);
	imwrite("tmp.png", dstbin);
	threshold_low = otsu_thresh_val * 0.4;
	threshold_high = threshold_low*3.0;// otsu_thresh_val * 1.5;
#endif
	...
	//return output5;//rgb image
	return output4;//gray image
#else
	return saveSWT;
	//return output1;
#endif//FLAG_ENABLE_BOOST
...
void SWTMedianFilter (Mat& SWTImage, std::vector<Ray> & rays) 
{
    ...
    for (auto& rit : rays) 
	{
        ...
        for (auto& pit : rit.points) 
		{
			tmp = std::min(pit.SWT, median);
			SWTImage.at<float>(pit.y, pit.x) = tmp;
			avgsw += tmp; num++;
        }
    }
	if(num > 0) avgsw /= num;
